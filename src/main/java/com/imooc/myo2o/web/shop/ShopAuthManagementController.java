...
public class ShopAuthManagementController {
	...
	private Map<String, Object> addShopAuthMap(HttpServletRequest request) {
		...
		ObjectMapper mapper = new ObjectMapper();
		ShopAuthMap shopAuthMap = null;
		String shopAuthMapStr = HttpServletRequestUtil.getString(request,
				"shopAuthMapStr");
		...
		if (shopAuthMap != null && shopAuthMap.getName() != null) {
			try {
				Shop currentShop = (Shop) request.getSession().getAttribute(
						"currentShop");
				shopAuthMap.setShopId(currentShop.getShopId());
				shopAuthMap.getEmployee().setShopOwnerFlag(0);
				shopAuthMap.getEmployee().setAdminFlag(0);
				ShopAuthMapExecution se = shopAuthMapService.addShopAuthMap2(shopAuthMap, profileImg);
	...
	@ResponseBody
	private Map<String, Object> removeShopAuthMap(Long shopAuthId,HttpServletRequest request) {
		Map<String, Object> modelMap = new HashMap<String, Object>();
		if (shopAuthId != null && shopAuthId > 0) {
			try {
				Shop currentShop = (Shop) request.getSession().getAttribute(
						"currentShop");
				ShopAuthMapExecution se = shopAuthMapService
						       .removeShopAuthMap2(shopAuthId, currentShop.getShopId());
				if (se.getState() == ShopAuthMapStateEnum.SUCCESS.getState()) {
					modelMap.put("success", true);
				} else {
					modelMap.put("success", false);
